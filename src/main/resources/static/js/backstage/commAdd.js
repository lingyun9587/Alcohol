...
$(function () {
    $("#submit").click(function () {
        ...
        if($(".ztphoto").find("img").attr("src")=="/images/backstage/uploadz.png"){
            alert("请选择商品主图");
            return;
        }
        var imageList="";//商品图片
        var weightList="";//商品权重
        var j=parseInt(0);
        $(".divImage ul li").each(function(i,e){
            var weight=$(this).find("img").attr("weight");//权重
            var shopImg = $(this).find("img").attr("src");//图片路径
            if(shopImg!="/images/backstage/upload.png"){
                if(j!=0){
                    imageList+=",";
                    weightList+=",";
                }
        ...
        editor.sync();
        var editorContent=$("#editor").val();//富文本编辑器中的html
        if(editorContent==''){
            alert("请上传商品详情");
            return;
        }
